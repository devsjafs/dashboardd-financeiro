...
async function checkContaAzulReceivable(accessToken: string, receivableId: string): Promise<{ isPaid: boolean; paidDate: string | null; dueDate: string | null; notFound: boolean } | null> {
  const resp = await fetch(`https://api.contaazul.com/v1/receivables/${receivableId}`, {
    method: 'GET',
  ...
  const paidDate = isPaid ? (item.payment_date || dueDate || new Date().toISOString().split('T')[0]) : null;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const lockKey = 'contaazul_sync_lock';
    ...
    try {
      ...
      const { data: connections } = await supabase.from('billing_connections').select('api_token, api_key, nome').eq('organization_id', orgId).eq('provider', 'contaazul');
      if (!connections || connections.length === 0) {
        return new Response(JSON.stringify({ error: 'Nenhuma conexão Conta Azul configurada.' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      ...
      for (let i = 0; i < boletos.length; i += BATCH_SIZE) {
        ...
        await Promise.all(batch.map(async (boleto) => {
          ...
          const recId = String(boleto.nibo_schedule_id);

          for (const conn of connections) {
            const accessToken = conn.api_token || conn.api_key;
            const result = await checkContaAzulReceivable(accessToken, recId);
            if (!result) continue;
      ...
      await supabase.from('audit_logs').insert({ organization_id: orgId, user_id: userId, action: 'contaazul_sync', details: { updated, unchanged, cancelled, dueDateUpdated, duration_ms: Date.now() - startTime } });
