...
async function checkSafe2PayTransaction(apiToken: string, transactionId: string): Promise<{ isPaid: boolean; paidDate: string | null; dueDate: string | null; notFound: boolean } | null> {
  const resp = await fetch(`https://api.safe2pay.com.br/v2/BankSlip/Get?Id=${transactionId}`, {
    method: 'GET',
  ...
  const detail = result?.ResponseDetail;
  if (!detail) return null;

  // Safe2Pay statuses: 1=Pendente, 3=Pago, 5=Cancelado, 6=Liberado, 11=Processando
  const isPaid = detail.TransactionStatus?.Id === 3 || detail.TransactionStatus?.Id === 6;
  const paidDate = isPaid ? (detail.PaymentDate?.split('T')[0] || new Date().toISOString().split('T')[0]) : null;
  const dueDate = detail.DueDate?.split('T')[0] || null;
  const isCancelled = detail.TransactionStatus?.Id === 5;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const lockKey = 'safe2pay_sync_lock';
    ...
    try {
      ...
      if (!boletos || boletos.length === 0) {
        return new Response(JSON.stringify({ updated: 0, unchanged: 0, message: 'Nenhum boleto pendente para sincronizar.' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }

      const { data: connections } = await supabase.from('billing_connections').select('api_token, api_key, nome').eq('organization_id', orgId).eq('provider', 'safe2pay');
      if (!connections || connections.length === 0) {
        return new Response(JSON.stringify({ error: 'Nenhuma conexão Safe2Pay configurada.' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      }
      ...
      for (let i = 0; i < boletos.length; i += BATCH_SIZE) {
        ...
        await Promise.all(batch.map(async (boleto) => {
          ...
          const txId = String(boleto.nibo_schedule_id);

          for (const conn of connections) {
            const apiToken = conn.api_token || conn.api_key;
            const result = await checkSafe2PayTransaction(apiToken, txId);
            if (!result) continue;
      ...
      await supabase.from('audit_logs').insert({ organization_id: orgId, user_id: userId, action: 'safe2pay_sync', details: { updated, unchanged, cancelled, dueDateUpdated, duration_ms: Date.now() - startTime } });
